...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('active', 'suspended', 'banned', 'frozen', name='userstatus'), nullable=False, server_default='active'),
    ...
    op.create_table(
        ...
        sa.Column('discount_type', sa.Enum('percentage', 'free_days', 'free_plan', name='discounttype'), nullable=False),
    ...
    op.create_index('ix_pages_facebook_page_id', 'pages', ['facebook_page_id'])
    ...
    op.create_table(
        ...
        sa.Column('plan_type', sa.Enum('free_trial', 'monthly', 'semi_annual', 'annual', 'custom', name='plantype'), nullable=False),
        sa.Column('status', sa.Enum('active', 'expired', 'cancelled', 'paused', name='subscriptionstatus'), nullable=False, server_default='active'),
    ...
    op.create_table(
        ...
        sa.Column('reply_type', sa.Enum('default', 'custom', 'random', name='replytype'), nullable=False, server_default='default'),
        ...
        sa.Column('dm_condition', sa.Enum('always', 'keywords', name='dmcondition'), nullable=False, server_default='always'),
    ...
    op.create_index('ix_campaigns_facebook_post_id', 'campaigns', ['facebook_post_id'])
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'replied', 'dm_sent', 'failed', 'skipped', name='logstatus'), nullable=False, server_default='pending'),
        sa.Column('error_message', sa.Text, nullable=True),
        sa.Column('dm_already_sent', sa.Boolean, nullable=False, server_default='false'),
        sa.Column('received_at', sa.DateTime(timezone=True), nullable=False),
        sa.Column('processed_at', sa.DateTime(timezone=True), nullable=True),
    )
    op.create_index('ix_comment_logs_facebook_comment_id', 'comment_logs', ['facebook_comment_id'])
    op.create_index('ix_comment_logs_commenter_facebook_id', 'comment_logs', ['commenter_facebook_id'])


def downgrade() -> None:
    """حذف جميع الجداول بالترتيب العكسي (مراعاة الـ foreign keys)"""
    ...
    sa.Enum(name='dmcondition').drop(op.get_bind())
    sa.Enum(name='replytype').drop(op.get_bind())
